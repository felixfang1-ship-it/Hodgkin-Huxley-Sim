...
function updateBuildupModel(dt) {
  ...
  // Buildup dissipates slowly. It clears faster after closure, matching the visual fade-return behavior.
  sodiumBuildup = clamp(sodiumBuildup - dt * (sodiumOpen ? 0.012 : 0.085), 0, 1);
  potassiumBuildup = clamp(potassiumBuildup - dt * (potassiumOpen ? 0.012 : 0.085), 0, 1);
  ...
  if (sodiumOpen && sodiumBuildup >= sodiumThreshold) {
    sodiumStateEl.value = 'inactivated';
...
function updateUI() {
  ...
  if (sodiumAutoClosed && sodiumState === 'inactivated') {
    sodiumStatusEl.innerHTML = 'Na<sup>+</sup>: auto-inactivated → buildup pressure';
    sodiumExplanationEl.innerHTML += ' The accumulated intracellular Na<sup>+</sup> residue exceeded the α/β-dependent closure threshold.';
  }

  if (potassiumAutoClosed && potassiumState === 'closed') {
    potassiumStatusEl.innerHTML = 'K<sup>+</sup>: auto-closed → buildup pressure';
    potassiumExplanationEl.innerHTML += ' The accumulated extracellular K<sup>+</sup> residue exceeded the α/β-dependent closure threshold.';
  }
