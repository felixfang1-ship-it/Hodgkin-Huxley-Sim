...
const OPEN_PHASE_DURATION = 15.0; // seconds of gradually slowing flux before forced closure
const AUTO_REOPEN_DELAY = 5.0; // seconds paused in closed state before reopening
...
function getSodiumFluxFactor() {
  ...
  // The exponent makes the visual flux taper smoothly to zero near high buildup.
  return Math.pow(clamp(1 - sodiumBuildup, 0, 1), 1.65);
}

function getPotassiumFluxFactor() {
  // Extracellular K+ buildup directly opposes outward K+ flux.
  // The exponent makes the visual flux taper smoothly to zero near high buildup.
  return Math.pow(clamp(1 - potassiumBuildup, 0, 1), 1.65);
}
...
function updateResidues(dt, t) {
  ...
  for (let i = residues.length - 1; i >= 0; i -= 1) {
    ...
    if (channelOpen) {
      ...
      // They do not fly back across the membrane.
      diffuseResidue(r, dt * 0.22, t);
      r.opacity -= dt * 0.18;
    }
...
function updateBuildupModel(dt) {
  ...
  if (sodiumConducting) {
    ...
    sodiumBuildup = clamp(sodiumOpenElapsed / OPEN_PHASE_DURATION, 0, 1);

    // Continuous residue production, but it gradually slows as buildup rises.
    sodiumResidueClock += dt * (0.65 + 4.25 * getSodiumFluxFactor());
    while (sodiumResidueClock >= 1) {
      spawnResidue('sodium');
      sodiumResidueClock -= 1;
    }

    if (sodiumOpenElapsed >= OPEN_PHASE_DURATION || getSodiumFluxFactor() <= 0.01) {
    ...
    sodiumBuildup = clamp(sodiumReopenTimer / AUTO_REOPEN_DELAY, 0, 1);
  ...
  if (potassiumConducting) {
    ...
    potassiumBuildup = clamp(potassiumOpenElapsed / OPEN_PHASE_DURATION, 0, 1);

    // Continuous residue production, but it gradually slows as buildup rises.
    potassiumResidueClock += dt * (0.65 + 4.25 * getPotassiumFluxFactor());
    while (potassiumResidueClock >= 1) {
      spawnResidue('potassium');
      potassiumResidueClock -= 1;
    }

    if (potassiumOpenElapsed >= OPEN_PHASE_DURATION || getPotassiumFluxFactor() <= 0.01) {
    ...
    potassiumBuildup = clamp(potassiumReopenTimer / AUTO_REOPEN_DELAY, 0, 1);
...
function updateUI() {
  ...
  if (sodiumAutoClosed && sodiumState === 'closed') {
    ...
    sodiumExplanationEl.innerHTML += ' Intracellular Na<sup>+</sup> buildup slowed the flux and forced the channel closed. It will reopen after the 5-second closed pause if the selector is still set to open.';
  }
  ...
  if (potassiumAutoClosed && potassiumState === 'closed') {
    ...
    potassiumExplanationEl.innerHTML += ' Extracellular K<sup>+</sup> buildup slowed the flux and forced the channel closed. It will reopen after the 5-second closed pause if the selector is still set to open.';
  }
