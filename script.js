...
// Potassium is the reference cycle: 15 s open/slowing, then 5 s closed/fading.
// Sodium closes 10x faster and reopens 12% faster than potassium.
const POTASSIUM_OPEN_PHASE_DURATION = 15.0;
const SODIUM_OPEN_PHASE_DURATION = POTASSIUM_OPEN_PHASE_DURATION / 10;
const POTASSIUM_REOPEN_DELAY = 5.0;
const SODIUM_REOPEN_DELAY = POTASSIUM_REOPEN_DELAY * 0.88;
...
function updateBuildupModel(dt) {
  ...
  if (sodiumConducting) {
    ...
    sodiumResidueClock += dt * (0.15 + 4.75 * getSodiumFluxFactor());
  ...
  if (potassiumConducting) {
    ...
    potassiumResidueClock += dt * (0.15 + 4.75 * getPotassiumFluxFactor());
...
function updateUI() {
  ...
  if (sodiumAutoClosed && sodiumState === 'closed') {
    ...
    sodiumExplanationEl.innerHTML += ' Intracellular Na<sup>+</sup> buildup slowed the flux and forced the channel closed. It will reopen after the faster sodium closed pause if the selector is still set to open.';
  }
  ...
  if (potassiumAutoClosed && potassiumState === 'closed') {
    ...
    potassiumExplanationEl.innerHTML += ' Extracellular K<sup>+</sup> buildup slowed the flux and forced the channel closed. It will reopen after the potassium closed pause if the selector is still set to open.';
  }
