...
const HH_CONSTANTS = {
  ...
  // Voltage clamp step used for the Iapp graph
  vHold: -65,
  vCommand: -20,
  stepTime: 1.0,
  stepTau: 0.15,

  // Classic HH-style constants
  cM: 1.0,
  eNa: 50,
  eK: -77,
  eL: -54.4,
...
function voltageAtTime(t) {
  if (t < HH_CONSTANTS.stepTime) return HH_CONSTANTS.vHold;

  const fraction = 1 - Math.exp(-(t - HH_CONSTANTS.stepTime) / HH_CONSTANTS.stepTau);
  return HH_CONSTANTS.vHold + (HH_CONSTANTS.vCommand - HH_CONSTANTS.vHold) * fraction;
}

function dVoltageDtAtTime(t) {
  if (t < HH_CONSTANTS.stepTime) return 0;

  return (
    (HH_CONSTANTS.vCommand - HH_CONSTANTS.vHold) /
    HH_CONSTANTS.stepTau *
    Math.exp(-(t - HH_CONSTANTS.stepTime) / HH_CONSTANTS.stepTau)
  );
}
...
function buildGateData() {
  ...
  for (let t = 0; t <= HH_CONSTANTS.tMax + 1e-9; t += HH_CONSTANTS.dt) {
    ...
    // Left graph: combined HH conductance terms.
    // Na+ combines the activation m gate with the inactivation h gate.
    const sodiumChannel = Math.pow(m, 3) * h;
    const potassiumChannel = Math.pow(n, 4);

    // Right graph: Iapp with capacitive current included.
    const vM = voltageAtTime(t);
    const dVmDt = dVoltageDtAtTime(t);
    const gNa = HH_CONSTANTS.gNaBar * sodiumChannel;
    const gK = HH_CONSTANTS.gKBar * potassiumChannel;

    const iNa = gNa * (vM - HH_CONSTANTS.eNa);
    const iK = gK * (vM - HH_CONSTANTS.eK);
    const iL = HH_CONSTANTS.gLBar * (vM - HH_CONSTANTS.eL);
    const iCap = HH_CONSTANTS.cM * dVmDt;
    const iApp = iCap + iNa + iK + iL;

    points.push({ t, sodiumChannel, potassiumChannel, iApp, iCap });
  }
  return points;
}
...
function drawGateGraph(points) {
  ...
  drawTrace(ctx, points, axes.xToPx, axes.yToPx, 'sodiumChannel', '#f15a3b', 3.5);
  drawTrace(ctx, points, axes.xToPx, axes.yToPx, 'potassiumChannel', '#70b82f', 3.5);
}
